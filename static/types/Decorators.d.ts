...
/**
 * Marks the template literal or service it is applied to as a SQL command.
 * When used as a decorator, it can only be applied to services of an entity that extends the `Database`
 * template.
 ...
 * as SQL code.
 * @param timeout       An optional timeout, in seconds, that overrides the default value of 60 seconds.
 */
declare function SQLCommand(timeout: number): <T extends Database>(target: T, key: string, descriptor: TypedPropertyDescriptor<(...args: any[]) => void | number>) => void;

/**
 * When applied to a service method, this marks that method as a SQL command, causing its contents to be parsed
 * as SQL code.
 */
declare function SQLCommand<T extends Database>(target: T, key: string, descriptor: TypedPropertyDescriptor<(...args: any[]) => void | number>): void;

/**
 * Marks the template literal or service it is applied to as a SQL query.
 * When used as a decorator, it can only be applied to services of an entity that extends the `Database`
 * template.
 ...
 * as SQL code.
 * @param timeout       An optional timeout, in seconds, that overrides the default value of 60 seconds.
 * @param maxRows       Controls how many rows are returned, overriding the default value of 500 rows.
 */
declare function SQLQuery(timeout: number, maxRows: number):
    <T extends Database, D extends DataShapeBase>(target: T, key: string, descriptor: TypedPropertyDescriptor<(...args: any[]) => INFOTABLE<D>>) => void;
