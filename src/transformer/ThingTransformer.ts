 ...
 */
const TypeScriptReturnPrimitiveTypes = [
...
Failed parsing at: \n${node.getText()}\n\n`);
    ...
    entityKindOfClassNode(classNode: ts.ClassDeclaration): TWEntityKind {
        ...
        if (!classNode.decorators || !classNode.decorators.length) {
            this.throwErrorForNode(
                classNode,
                `Ambiguous class kind ${classNode.name}. Thingworx classes must extend from DataShape, ThingShape or have an entity kind decorator.`,
            );
        }

        const isThing = classNode.decorators.some(
            (decorator) => decorator.expression.kind == ts.SyntaxKind.Identifier && decorator.expression.getText() == 'ThingDefinition',
        );
        const isThingTemplate = classNode.decorators.some(
            (decorator) =>
                decorator.expression.kind == ts.SyntaxKind.Identifier && decorator.expression.getText() == 'ThingTemplateDefinition',
        );
        const isDataShape = classNode.decorators.some(
            (decorator) => decorator.expression.kind == ts.SyntaxKind.Identifier && decorator.expression.getText() == 'DataShapeDefinition',
        );
        ...
        if (this.after) {
            ...
            if (!this.isGlobalCode) {
                ...
                if (
                    ...
                    transpiledNode.original &&
                    transpiledNode.original.kind == ts.SyntaxKind.MethodDeclaration
                ) {
            ...
            for (const arg of organizations) {
                if (arg.kind == ts.SyntaxKind.PropertyAccessExpression) {
                    ...
                    if (expression.expression.getText() != 'Organizations') {
                        this.throwErrorForNode(
                            arg,
                            `Organizations specified in the @${kind} decorator must be accessed from the Organizations collection or via the "Unit" function.`,
                        );
                    }
                    ...
                    if (callExpression.expression.getText() != 'Unit') {
                        this.throwErrorForNode(
                            arg,
                            `Organization units in the @${kind} decorator  must be specified via the "Unit" function.`,
                        );
                    }
                    ...
                    if (
                        organizationArg.kind != ts.SyntaxKind.PropertyAccessExpression ||
                        organizationExpression.expression.getText() != 'Organizations'
                    ) {
                        this.throwErrorForNode(
                            arg,
                            `Organizations specified in the "Unit" function must be accessed from the Organizations collection.`,
                        );
                    }
    ...
    permissionsOfNode(node: ts.Node, resource = '*'): TWExtractedPermissionLists {
        ...
        const decorators = node.decorators?.filter(
            (d) =>
                d.expression.kind == ts.SyntaxKind.CallExpression &&
                PermissionDecorators.includes((d.expression as ts.CallExpression).expression.getText()),
        );
        ...
        for (const decorator of decorators) {
            ...
            if (!callExpression.arguments.length)
                this.throwErrorForNode(node, `A permission decorator must have at least one user or group and at least one permission`);
            ...
            for (const permission of permissions) {
                permissionList[permissionKind]![targetResource][permission] = principals.map((p) => ({
                    principal: p.name,
                    type: p.type,
            ...
            for (const kind in val) {
                ...
                for (const resource in val[kind]!) {
                    ...
                    for (const key of Object.keys(val[kind]![resource])) {
                        ...
                        if (
                            value.filter((p, i) => value.find((p2, i2) => i != i2 && p2.type == p.type && p2.principal == p.principal))
                                .length
                        ) {
                            this.throwErrorForNode(
                                node,
                                `Each user or group may only appear a single time per permission in a permissions decorator.`,
                            );
                        }
    ...
    getGlobalFileClass(node: ts.SourceFile): string | undefined {
        ...
        if (
            ...
            (firstStatement as ts.ExpressionStatement).expression.kind != ts.SyntaxKind.StringLiteral
        )
            return undefined;
        ...
        if (literal == 'use strict') {
            ...
            if (
                secondStatement.kind != ts.SyntaxKind.ExpressionStatement ||
                (secondStatement as ts.ExpressionStatement).expression.kind != ts.SyntaxKind.StringLiteral
            )
                return undefined;
    ...
    visitRootNode(node: ts.Node) {
            ...
            if (
                ...
                if (literalComponents.length == 2 && literalComponents[0] == 'use' && literalComponents[1] != 'strict') {
                    ...
                    if (!this.experimentalGlobals) {
                        this.throwErrorForNode(
                            node,
                            'Experimental support for global code must be enabled in order to declare global symbols.',
                        );
                    }
                    ...
                    if (!this.store) {
                        this.throwErrorForNode(
                            node,
                            'Experimental support for global code is not compatible with your build script. Ensure the gulpfile is up to date.',
                        );
                    }
        ...
        if (node.kind == ts.SyntaxKind.ClassDeclaration) {
            ...
            if (heritage.expression.kind == ts.SyntaxKind.Identifier) {
                ...
                if (
                    callNode.expression.getText() != 'ThingTemplateWithShapes' &&
                    callNode.expression.getText() != 'ThingTemplateReference' &&
                    callNode.expression.getText() != 'ThingTemplateWithShapesReference'
                ) {
                    this.throwErrorForNode(
                        node,
                        `Unknown base class for ${classNode.name}. Thing and ThingTemplate classes must extend from a ThingTemplateWithShapes(...) expression, a ThingTemplateWithShapesReference(...) expression, a ThingTemplateReference(...) expression or a base ThingTemplate class.`,
                    );
                }
                ...
                if (
                    ...
                    if (!callNode.arguments.length) {
                        this.throwErrorForNode(
                            node,
                            `The ThingTemplateWithShapes(...) expression must have at least one ThingTemplate parameter.`,
                        );
                    }
                    ...
                    if (thingTemplateNode.kind == ts.SyntaxKind.CallExpression) {
                        ...
                        switch (callExpression.expression.getText()) {
                            ...
                            default:
                                this.throwErrorForNode(
                                    heritage,
                                    `Unknown thing template expression "${callExpression.expression.getText()}" used in ThingTemplateWithShapes(...)`,
                                );
                        }
                    } else {
                        this.thingTemplateName =
                            callNode.arguments[0].kind == ts.SyntaxKind.StringLiteral
                    ...
                    if (callNode.arguments.length != 1)
                        this.throwErrorForNode(
                            node,
                            `The ThingTemplateReference(...) expression must have a single string literal parameter.`,
                        );

                    if (callNode.arguments[0].kind != ts.SyntaxKind.StringLiteral)
                        this.throwErrorForNode(
                            node,
                            `The ThingTemplateReference(...) expression must have a single string literal parameter.`,
                        );
            ...
            this.published =
                ...
                classNode.decorators.some(
                    (decorator) => decorator.expression.kind == ts.SyntaxKind.Identifier && decorator.expression.getText() == 'published',
                );
            ...
            this.editable =
                !!classNode.decorators &&
                classNode.decorators.some(
                    (decorator) => decorator.expression.kind == ts.SyntaxKind.Identifier && decorator.expression.getText() == 'editable',
                );
            ...
            if (!this.watch) {
                ...
                this.runtimePermissions = this.mergePermissionListsForNode(
                    [this.runtimePermissions].concat(this.permissionsOfNode(classNode)),
                    node,
                );
                ...
                if (
                    this.instanceVisibilityPermissions.length &&
                    this.entityKind != TWEntityKind.ThingShape &&
                ...
                case 'members':
                    ...
                    orgUnit.members = members.elements.map((m) => {
                        if (m.kind != ts.SyntaxKind.PropertyAccessExpression) {
                            this.throwErrorForNode(
                                members,
                                `Organizational unit members must be specified via the "Users" and "Groups" collections.`,
                            );
                        }
            ...
            if (node.initializer.kind == ts.SyntaxKind.ObjectLiteralExpression) {
                ...
                for (const member of objectLiteral.properties) {
                    ...
                    if (assignment.initializer.kind == ts.SyntaxKind.PropertyAccessExpression) {
                        ...
                        user.extensions[member.name.text] = this.constantValueOfExpression(
                            assignment.initializer as ts.PropertyAccessExpression,
                        );
                        ...
                        user.extensions[member.name.text] =
                            (assignment.initializer as ts.LiteralExpression).text || assignment.initializer.getText();
                    }
                }
    ...
    visitDataShapeField(node: ts.PropertyDeclaration) {
        ...
        if (ordinal) {
            if (!parseInt(ordinal))
                this.throwErrorForNode(
                    node,
                    `Non numeric value specified in ordinal decorator for property ${property.name}: ${property.baseType}`,
                );
        ...
        if (TWBaseTypes[baseType] == 'INFOTABLE') {
            ...
            if (typeArguments) {
                if (typeArguments.length != 1)
                    this.throwErrorForNode(node, `Unknown generics specified for property ${property.name}: ${property.baseType}`);
        ...
        if (node.initializer) {
            if (node.initializer.kind == ts.SyntaxKind.PropertyAccessExpression) {
                ...
                node.initializer.expression.escapedText == 'Date'
            ) {
                if (
    ...
    visitProperty(node: ts.PropertyDeclaration) {
            ...
            */
            ...
            if (remoteArguments.length == 2) {
                ...
                for (const bindingAspect of bindingAspects.properties) {
                    ...
                    switch (name) {
                        case 'pushType':
                            property.remoteBinding.pushType = (bindingAspect.initializer as ts.StringLiteral)
                                .text as TWPropertyRemotePushKind;
                            break;
                        case 'pushThreshold':
                            property.remoteBinding.pushThreshold = bindingAspect.initializer.getText();
                            break;
                        case 'startType':
                            property.remoteBinding.aspects.startType = (bindingAspect.initializer as ts.StringLiteral)
                                .text as TWPropertyRemoteStartKind;
                            break;
                        case 'foldType':
                            property.remoteBinding.foldType = (bindingAspect.initializer as ts.StringLiteral)
                                .text as TWPropertyRemoteFoldKind;
                            break;
                        case 'cacheTime':
        ...
        if (this.hasDecoratorNamed('local', node)) {
            ...
            for (const argument of localArguments) {
                if (argument.kind != ts.SyntaxKind.StringLiteral)
                    this.throwErrorForNode(node, 'The local binding decorator arguments must be string literals.');
            }
        ...
        if (node.parameters.length > 1) {
            ...
            if (argList.type.kind == ts.SyntaxKind.TypeReference) {
                ...
                if (symbol) {
                    ...
                    const declarations = literalType
                        .getProperties()
                        .flatMap((p) => p.declarations)
                        .filter((d) => !!d) as ts.Declaration[];
            ...
            for (const arg of args.elements) {
                ...
                if (arg.initializer) {
                    if (arg.initializer.kind == ts.SyntaxKind.PropertyAccessExpression) {
                        ...
                        ts.isNewExpression(arg.initializer) &&
                        ts.isIdentifier(arg.initializer.expression) &&
                        arg.initializer.expression.escapedText == 'Date'
                    ) {
                        if (
                            arg.initializer.arguments &&
                            arg.initializer.arguments.length == 1 &&
                            ts.isStringLiteral(arg.initializer.arguments[0])
                        ) {
                            parameter.aspects.defaultValue = new Date(arg.initializer.arguments[0].text).toISOString();
                        } else {
                            this.throwErrorForNode(
                                node,
                                'Exactly one string literal argument is required when setting Date as default value.',
                            );
                        }
                    } else {
                ...
                if (TWBaseTypes[baseType] == 'INFOTABLE') {
                    ...
                    if (typeArguments) {
                        if (typeArguments.length != 1)
                            this.throwErrorForNode(
                                node,
                                `Unknown generics specified for parameter ${parameter.name}: ${parameter.baseType}`,
                            );
                ...
                if (TWBaseTypes[baseType] == 'INFOTABLE') {
                    ...
                    if (typeArguments) {
                        if (typeArguments.length != 1)
                            this.throwErrorForNode(
                                originalNode,
                                `Unknown generics specified for service result: ${service.resultType.baseType}`,
                            );

                        if (typeArguments[0].kind == ts.SyntaxKind.LiteralType) {
                            service.resultType.aspects.dataShape = (
                                (typeArguments[0] as ts.LiteralTypeNode).literal as ts.StringLiteral
                            ).text;
                        } else {
    ...
    visitSubscription(node: ts.MethodDeclaration): ts.MethodDeclaration {
        ...
        if (this.hasDecoratorNamed('localSubscription', node)) {
            ...
            for (const arg of localArguments) {
                if (arg.kind != ts.SyntaxKind.StringLiteral)
                    this.throwErrorForNode(node, 'The arguments of the localSubscription decorator must be string literals.');
            }
            ...
            for (const arg of localArguments) {
                if (arg.kind != ts.SyntaxKind.StringLiteral)
                    this.throwErrorForNode(node, 'The arguments of the subscription decorator must be string literals.');
            }
    ...
    transpiledBodyOfThingworxMethod(node: ts.FunctionDeclaration): string {
        ...
        return (
            [...helpersToInline].map((helper) => codeMap[helper.name] || helper.text).join('\n\n') +
    ...
    visitConfigurationTablesDefinition(node: ts.ClassExpression) {
        ...
        for (const member of node.members) {
            if (member.kind == ts.SyntaxKind.MethodDeclaration)
                this.throwErrorForNode(node, `The @ConfigurationTables class cannot contain methods.`);
            ...
            if (!type.typeArguments || type.typeArguments.length != 1)
                this.throwErrorForNode(
                    member,
                    `Configuration table properties must have one type parameter representing the data shape name.`,
                );
            ...
            if (typeArgument.kind == ts.SyntaxKind.TypeReference) {
                ...
                this.throwErrorForNode(
                    member,
                    `The configuration table type argument must be a data shape class reference or exported name.`,
                );
            }
    ...
    visitConfiguration(node: ts.ObjectLiteralExpression) {
        ...
        for (const member of node.properties) {
            ...
            if (member.kind != ts.SyntaxKind.PropertyAssignment)
                this.throwErrorForNode(node, `The @config object must contain only property assignments.`);
    ...
    private extractObjectLiteral(literal: ts.ObjectLiteralExpression): Record<string, unknown> {
        ...
        for (const member of literal.properties) {
            ...
            if (!member.name || member.name.kind != ts.SyntaxKind.Identifier)
                this.throwErrorForNode(member, `Configuration field names cannot be computed.`);
    ...
    toXML(): string {
        ...
        if (this.entityKind != TWEntityKind.ThingShape) {
            entity.$[this.entityKind == TWEntityKind.Thing ? 'thingTemplate' : 'baseThingTemplate'] =
                this.thingTemplateName || 'GenericThing';
        ...
        for (const property of this.properties) {
            ...
            if (property.remoteBinding) {
                ...
                if (property.remoteBinding.aspects)
                    for (const key in property.remoteBinding.aspects) {
            ...
            if (property.localBinding) {
                ...
                if (property.localBinding.aspects)
                    for (const key in property.localBinding.aspects) {
        ...
        if (this.runtimePermissions.runtime) {
            ...
            for (const resource in this.runtimePermissions.runtime) {
                ...
                for (const permission in this.runtimePermissions.runtime[resource]) {
                    const principals = this.runtimePermissions.runtime[resource][permission].map((p) => ({
                        $: { name: p.principal, type: p.type, isPermitted: p.isPermitted },
                    }));
        ...
        if (this.runtimePermissions.runtimeInstance) {
            ...
            for (const resource in this.runtimePermissions.runtimeInstance) {
                ...
                for (const permission in this.runtimePermissions.runtimeInstance[resource]) {
                    const principals = this.runtimePermissions.runtimeInstance[resource][permission].map((p) => ({
                        $: { name: p.principal, type: p.type, isPermitted: p.isPermitted },
                    }));
                    permissionDefinition[permission] = [{ Principal: principals }];
                }
        ...
        if (this.instanceVisibilityPermissions.length) {
            ...
            entity.InstanceVisibilityPermissions[0].Visibility[0] = {
                Principal: this.instanceVisibilityPermissions.map((p) => ({ $: p })),
            };
        }
    ...
    toDeclaration(): string {
        if (this.entityKind && this.className) {
            if (this.entityKind == TWEntityKind.Thing) {
                ...
                const users = `declare interface Users { ${Object.values(this.users)
                    .map((u) => `${u.name}: UserEntity;`)
                    .join(' ')} }\n\n`;
                const groups = `declare interface Groups { ${Object.values(this.userGroups)
                    .map((u) => `${u.name}: GroupEntity;`)
                    .join(' ')} }\n\n`;
                return users + groups;
            } else if (this.entityKind == TWEntityKind.Organization) {
                return `declare interface ${this.entityKind}s { ${JSON.stringify(this.exportedName)}: ${this.entityKind}Entity<${
                    this.orgUnits.map((u) => JSON.stringify(u.name)).join(' | ') || 'string'
                }>}`;
            } else {
                return `declare interface ${this.entityKind}s { ${JSON.stringify(this.exportedName)}: ${this.entityKind}Entity<${
                    this.className
...
export function TWThingTransformerFactory(program: ts.Program, root: string, after = false, watch = false, project?: string | TWConfig) {
