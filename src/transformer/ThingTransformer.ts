...
export class TWThingTransformer implements TWCodeTransformer {
     ...
     */
    hasDecoratorNamed(name: string, node: ts.HasDecorators): boolean {
     ...
     */
    literalArgumentOfDecoratorNamed(name: string, node: ts.HasDecorators): string | undefined {
     ...
     */
    numericArgumentOfDecoratorNamed(name: string, node: ts.HasDecorators): string | undefined {
        if (!this.hasDecoratorNamed(name, node)) return;
     ...
     */
    identifierArgumentOfDecoratorNamed(name: string, node: ts.HasDecorators): string | undefined {
        if (!this.hasDecoratorNamed(name, node)) return;
     ...
     */
    visibilityPermissionsOfKindForNode(kind: string, node: ts.HasDecorators): TWVisibility[] {
    ...
    visitRootNode(node: ts.Node) {
        ...
        if (node.kind == ts.SyntaxKind.ClassDeclaration) {
            ...
            if (ts.canHaveDecorators(node) && this.hasDecoratorNamed('ifenv', node)) {
    ...
    return function TWThingTransformerFunction(context: ts.TransformationContext) {
        ...
        return (node: ts.SourceFile) => {
            ...
            try {
                ...
                return ts.visitNode(node, node => targetTransformer.visit(node));
